...
# Load the CSV data
data <- read_csv(
  "bacteriophage_data.csv",
  col_types = cols(
    Bacteriophage = col_character(),
...
data <- data %>%
  mutate(
    bacteriophage = factor(Bacteriophage),
...
p <- ggplot(data, aes(x = Bacteriophage,
...
ggsave(
  filename = "bacteriophage_tile_plot_with_parameters.tif",
  plot = p,
  width = 5.5, height = 4.2, dpi = 300, units = "in",
  compression = "lzw"
)
