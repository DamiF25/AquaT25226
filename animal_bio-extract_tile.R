...
# Load the CSV data
data <- read_csv(
  "animal_bioextract_data.csv",
  col_types = cols(
    `Animal bio-extract` = col_character(),
...
data <- data %>%
  mutate(
    `Animal bio-extract` = factor(`Animal bio-extract`),
...
p <- ggplot(data, aes(x = `Animal bio-extract`,
...
ggsave(
  filename = "animal_bio-extract_tile_plot_with_parameters.tif",
  plot = p,
