...
# Load the CSV data
data <- read_csv(
  "microbial_extract_data.csv",
  col_types = cols(
    `Microbial extract` = col_character(),
...
data <- data %>%
  mutate(
    `Microbial extract` = factor(`Microbial extract`),
...
p <- ggplot(data, aes(x = `Microbial extract`,
...
ggsave(
  filename = "microbial_extract_tile_plot_with_parameters.tif",
  plot = p,
  width = 10.5, height = 6.3, dpi = 600, units = "in",
  compression = "lzw"
)
