...
# Read CSV data
df <- read.csv("Antibiotic_control.csv", stringsAsFactors = FALSE)
...
my_colors <- c("Included" = "#60435D", "Not included" = "#FFB62E")

# Build donut chart
donut_plot <- ggplot(summary_df, aes(x = 2, y = Percent, fill = Antibiotic)) +
...
###############################################
ggsave(
  filename = "Antibiotic_control_percentage.tif",
  plot = donut_plot,
  width = 9,
  height = 6,
  dpi = 600,
